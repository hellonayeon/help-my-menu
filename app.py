...
db = client.dbtest # DB제출 전 경로 변경할 것!
...
def recipe_list():
    recipe_list = list(db.recipe_basic_list.find({},{'_id':False}))
...
def detail_recipe():
    ...
    recipe_process = list(db.recipe_process.find({'RECIPE_ID':recipe_id},{'_id':False}))
    recipe = db.recipe_basic_list.find_one({'RECIPE_ID':recipe_id}, {"_id": False})
    return jsonify({'recipe_process':recipe_process, 'recipe':recipe})
